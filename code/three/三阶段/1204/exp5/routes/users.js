...
var {aesEncrypt,keys} = require("../utils/aes");
...
router.post("/zhuce",(req,res)=>{
  ...
  var username = aesEncrypt(req.body.username,keys);
    ...
    waterfall([
      ...
      (args,callback)=>{
        ...
        if(result.type=="1"){
          // 注册成功
          // res.send(result.msg) 
          res.send(`<script>alert('恭喜你,注册成功!直接前往登陆!');location.href='/login?username=${username}' </script>`)
        }else{
          // 注册失败
          // res.send(result.msg) 
  ...
  conn((err,db)=>{
    ...
    user.find({username:body.username,password:body.password},{}).toArray((err,result)=>{
      ...
      if(result.length>0){
        ...
        res.redirect("/");  // 重定向 到  首页 
        console.log(req.session);
      }else{
        //登录失败 
        //  res.send("用户名或者密码错误!")
        res.send(`<script>alert('用户名或者密码错误,请重新登录');location.href='/login?username=${aesEncrypt(body.username,keys)}' </script>`)
      }
    })
  })
